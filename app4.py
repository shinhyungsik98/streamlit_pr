...
def main() :
    ...
    #체크박스 : 둘중에 하나만 선택하게끔 만들떄.(체크/헤제)
    #체크하면, 헤드 5개 보여주고 , 해제하면 안보여주도록

    if st.checkbox('헤드5개보기') :
    ...
    if my_choice == language[0] or my_choice==[2]:
    ...
    #슬라이더 : 숫자 조정하는데 주로 사용
    st.slider('데이터선택', -5.0,10.5,0.0,0.5)

    #나이를 슬라이더로 입력받는다.
    #1세부터 120세 까지 입력받을수 있도록 한다.
    #선택한 나이가 웹화면에 출력되도록 한다. "선택한 나이는 33세입니다."
    age = st.slider('나이선택',1,120,1,1)
